        ...
        return (
            <div className={`relative overflow-hidden glass-panel bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 group`}>
                ...
                <div className="flex justify-between items-start mb-4">
                    ...
                    <div className={`px-2 py-1 rounded-full text-xs font-bold uppercase flex items-center gap-1 border ${isActive ? 'bg-green-500/20 text-green-300 border-green-500/50' : 'bg-red-500/20 text-red-300 border-red-500/50'}`}>
                        {isActive ? <><CheckCircle size={12} /> Active</> : <><AlertCircle size={12} /> Empty</>}
                    </div>
                </div>

                <div className="space-y-4">
                    <div className="relative border-2 border-dashed border-white/30 rounded-lg p-6 text-center hover:border-white/60 transition-colors bg-black/20">
                        <input
                            type="file"
